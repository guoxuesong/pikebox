...
class Wallet{
	...
	string sendtoaddress(string address,float amount)/*{{{*/
	{
		while(1){
#ifdef UNITTEST_ONLY
			if(amount>MANUAL_AMOUNT_MAX){
...
#endif
			string res=run(sprintf("%s sendtoaddress %s %0.8f",cli,address,amount),1)&&(res/"\n")[0];
			if(error&&error->code==-13&&passphase){
				run(sprintf("%s walletpassphrase %s 3600",cli,passphase),1);
				continue;
			}
	...
	int move(string from,string to,float amount)/*{{{*/
	{
		while(1){
#ifdef UNITTEST_ONLY
			if(amount>MANUAL_AMOUNT_MAX){
...
#endif
			string res=run(sprintf("%s move %q %q %0.8f",cli,from,to,amount),1)&&(res/"\n")[0];
			if(error&&error->code==-13&&passphase){
				run(sprintf("%s walletpassphrase %s 3600",cli,passphase),1);
				continue;
			}
	...
	object getmininginfo()/*{{{*/
	{
		return run(sprintf("%s getmininginfo",cli))&&Save.load(MiningInfo(),res);
	}/*}}}*/
	array listtransactions(string|void account,int|void count,int|void from)/*{{{*/
	{
		if(!zero_type(from))
			return run(sprintf("%s listtransactions %q %d %d",cli,account,count,from))&&res;
		else if(!zero_type(count))
			return run(sprintf("%s listtransactions %q %d",cli,account,count))&&res;
		else if(!zero_type(account))
			return run(sprintf("%s listtransactions %q",cli,account))&&res;
		else
	...
	class BtcChina{//not works
		int buy_many(string btcltc)/*{{{*/
		{
			...
			while(1){
				...
				werror("ticker=%O\n",ticker);
				float amount=Norm.floor(money_free/(float)ticker->ticker->last,BTC_PRECISION);
				mapping res=r->buyIcebergOrder(inst,0,amount,max(0.001,Norm.floor(amount/8,BTC_PRECISION)),0.1);
				r->update_account();
		...
		int sell_many(string btcltc)/*{{{*/
		{
			...
			while(1){
				...
				mapping res=r->sellIcebergOrder(inst,0,amount,max(0.001,Norm.floor(amount/8,BTC_PRECISION)),0.1);
				r->update_account();
...
class RequesterAbstraceLayerMode{
	...
	class OkCoin{
		...
		int safe_order_market(string btcltc,string type,float percent,SafeOrderMarketOptions|void options)/*{{{*/
		{
			...
			if(type=="buy"){
				float money_free=account->money_free;
				float amount=Norm.floor((money_free)*percent/(float)ticker->ticker->last,BTC_PRECISION);
				float money;
				...
				float amount=Norm.floor(amount_free*percent,BTC_PRECISION);
				if(options->penny&&amount>=min_amount){
					amount=min_amount;
				}
		...
		int safe_order_market(string btcltc,string type,float percent,SafeOrderMarketOptions|void options)/*{{{*/
		{
			...
			if(type=="buy"){
				...
				if(options->penny&&amount>=min_amount&&money>=min_money){
					...
					if(money<min_money){
						money=min_money;
						amount=Norm.floor(min_money/(float)ticker->ticker->last,BTC_PRECISION);
					}
				}else{
					money=Norm.floor(money_free*percent,CNY_PRECISION);
				}
		...
		int safe_order_market(string btcltc,string type,float percent,SafeOrderMarketOptions|void options)/*{{{*/
		{
			...
			if(btcltc=="btc"){
				if(type=="buy"){
					...
					float amount=Norm.floor((money/currprice),BTC_PRECISION);
					if(amount>0){
						if(options->check_account_only)
							return 1;
						...
						nextaccount->inst2position[inst]->amount_free=Norm.round(nextaccount->inst2position[inst]->amount_free,BTC_PRECISION);
						nextaccount->inst2position[inst]->amount=Norm.round(nextaccount->inst2position[inst]->amount,BTC_PRECISION);
					...
					float amount=Norm.floor(account->inst2position[inst]->amount_free*percent,BTC_PRECISION);
...
class EA{
	...
	void on_timer_psig(array rr,object wallet,int psig,string pwhy,float|void currprice)
	{
		if(psig==-1){
			...
			float one=Norm.floor(all/sizeof(rr),BTC_PRECISION);
...
int ea_main(int argc,array argv)
{
	...
	tt+=({Thread.Thread(ea->ea_prepare,rr,manual_flag?BitCoinUnitTestWallet():BitCoinWallet(),in,out)});
	foreach(rr,object r){
		in_queues+=({in=Thread.Queue()});
		out_queues+=({out=Thread.Queue()});
