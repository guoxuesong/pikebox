...
//!   extra modifier: stdout_watcher stderr_watcher. add by Xuesong Guo.
//!   function(string:void) stdout_watcher,stderr_watcher; 
//!   all data pass to stdout and stderr will be passed to these two watchers
//!		function(Process:void) handle_process;
...
mapping run(string|array(string) cmd, void|mapping modifiers)
{
  ...
  function stdout_watcher=modifiers->stdout_watcher;
  function stderr_watcher=modifiers->stderr_watcher;
  ...
  if(stringp(modifiers->stdin))
  {
	...
	modifiers->handle_process&&modifiers->handle_process(p);
  ...
  mystdout->set_read_callback( lambda( mixed i, string data) { 
		  stdout_watcher&&stdout_watcher(data);
                                 gotstdout += data; 
                               } );
  mystderr->set_read_callback( lambda( mixed i, string data) {
		  stderr_watcher&&stderr_watcher(data);
                                 gotstderr += data;
                               } );
