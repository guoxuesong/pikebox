...
//!
//! @note
//!   Modified to use GdbmCluster and Cross. Xuesong Guo
...
class RobotExcluder
{
  ...
  int got_reply;
  int failed;
  array(string) reject_globs/*=({ })*/; //make check not usable if download not complete
  string host;
...
{
  Queue queue;
  GdbmCluster.GdbmCluster thread_handler;
  function fetch_cb, done_cb, error_cb;
  function prepare_cb;
		    ...
		    void|mapping _headers)
  {
    ...
    else
    {
      ...
      {
	int got;
	while(thread_handler->outq->size()){
		werror("wait\n");
		string real_uri=thread_handler->wait();
		werror("got %s\n",real_uri);
		string links=(thread_handler[real_uri]);
		object r=Standards.URI(real_uri);
		if(links==0)
		{
			error_cb(real_uri, 1100, ([]),@args);
			if(queue->get_stage(r)<=1)
				queue->set_stage(r,6);
		}else{
			if(links!=""){
				add_links(map(links/"\n",Standards.URI));
			}
			thread_handler->delete(real_uri);
		}
		queue->stats->close_callback(r);
		got=1;
	}
	if(!got){
    ...
    if(objectp(uri))
    {
	  ...
	  thread_handler[(string)real_uri]=Function.curry(fetch_cb)(uri,headers,@args);
...
//!    Standards.URI uri, mapping headers, mixed ... args. 
//!    should return a string containing additional links found within data, each link one line,
//!    that will be analyzed for insertion into the crawler queue (assuming
//!    they are allowed by the allow/deny rulesets.
//!    _fetch_cb run in other threads, you can use S{} in it to run on other muchines.
//!  @param _error_cb
...
//!    functions.
  void create(string|GdbmCluster.GdbmCluster workdir,Queue _queue,
	      function _fetch_cb, function _error_cb,
	      function _done_cb, function _prepare_cb, 
	      string|array(string)|Standards.URI|
	      array(Standards.URI) start_uri,
	      mixed ... _args)
  {
    thread_handler=stringp(workdir)?GdbmCluster.GdbmCluster(workdir,1,128):workdir;
    queue=_queue;
    args=_args;
    fetch_cb=_fetch_cb;
    error_cb=_error_cb;
...
int main()
{
	constant WINDOW_WIDTH_NOTUSED=32;
	constant GRANULARITY_NOTUSED=1;
	object queue=MemoryQueue(Stats(WINDOW_WIDTH_NOTUSED,GRANULARITY_NOTUSED),Policy(),0,0);
	mkdir("/tmp/crawler_data");
	cd("/tmp/crawler_data");
	Cross.HOSTS((["localhost":1]));
	object crawler=Crawler("/tmp/crawler",queue,lambda(Standards.URI uri, mapping headers){
			S{
				Process.run(({"wget",STRING{(string)uri}}));
				//Process.system(sprintf("wget %s",STRING{(string)uri}));
			}
			if((string)uri=="http://www.google.cn")
				return "http://www.baidu.com\nhttp://www.guoxuesong.com";
			return "";
			},lambda(Standards.URI real_uri, int status_code, mapping headers){
			werror("error: %s %d\n",(string)real_uri,status_code);
			},lambda(){werror("done\n");exit(0);},0,"http://www.google.cn",);
	return -1;
}
