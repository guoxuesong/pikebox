MIXIN Session{
extern void _show(Widget w,string title);
extern void _reshow(Widget w);
extern function write;
extern void add_dnd_rule(WidgetType,PositionType,RuleType,string);
extern int server_mode_flag;
extern void update_syncers();
extern int command(string cmd,array args);
extern string handle_todolist_cmd(string cmd,mixed ... args);
//extern PUBLIC _session_timeout_(mixed ... args);
int flag_show;
string save_title;
//extern void start_dnd();

private int from_gui_flag=0;

Widget curr_widget;

array global_todolist=({});

void show(Widget w,string title)
{
	show_flag_percmd=1;
	werror("SHOW\n");
	WIDGETD->clear_todolist_sn++;
	flag_show=1;
	save_title=title;
	curr_widget=w;
	// 不知道为什么先前写成这样 可能是server_mode_flag的需要，所以我把这3行加入到server_mode_flag
	update_syncers();
	w->clear_todolist();
	ASSERT(sizeof(w->todo)==0);
	w->todo+=global_todolist;
	if(server_mode_flag){
		write("%s\n",join(({"remote_show",encode_value(w),title})," "));
	}else if(from_gui_flag==0){
		werror("_SHOW\n");
		_show(w,title);
	}else{
		werror("_RESHOW\n");
...
#if 0
array curr_widget_handlers()/*{{{*/
{
	array res=({});
	void walk(Widget p)
	{
		if(widget2handler[p]){
			res+=({widget2handler[p]});
		}
		foreach(p->items;Widget pp;int one) {
			walk(pp);
		}
	};
	if(curr_widget)
...
{
	global_todolist+=({({"location",url})});
}
PUBLIC open_window(string url,string|void target)
{
	global_todolist+=({({"open_window",url,target||"_blank"})});
}
...
PUBLIC refresh()/*{{{*/
{
	...
	if(from_gui_flag){
		if(curr_widget){
			drain_todolist(curr_widget);
		}
		foreach(global_todolist,array a){
			handle_todolist_cmd(@a);
		}
		global_todolist=({});
	}else{
		if(curr_widget){
			werror("CALL show\n");
			show(curr_widget,save_title);
		}
	}
}/*}}}*/

PUBLIC cancel_errorprompt()
{
	if(curr_widget){
...
int show_flag_percmd;

int cmd_sn;
PUBLIC from_gui(int sn,mixed ... args)/*{{{*/
{
	show_flag_percmd=0;
	int ok;
	if(sn==-1){
		...
		werror("INFO: recieve from_gui %d, want %d, %d-%d will be ignored\n",sn,cmd_sn,cmd_sn,sn);
		cmd_sn=sn+1;
	}else if(sn==0){
		werror("INFO: looks cmd_sn reseted to 0\n");
		cmd_sn=sn+1;
		ok=1;
	}
	if(!ok){
		werror("INFO: command ignored\n");
		return 0;
	}
	if(flag_show==0){
		args=({"_session_timeout_"})+args;
	}
	werror("FROM_GUI! %d +1\n",from_gui_flag);
	from_gui_flag++;
	int err;
	...
	if(e||err){
		Widget res=WIDGETD->show_error(curr_widget,e,err);
		if(res)
			show(res,"错误");
		else
			refresh();
...
mapping id2widget=WIDGETD->id2widget;
//mapping widget2onclick=WIDGETD->widget2onclick;

//object allow=RuleType("allow");
//object deny=RuleType("deny");


//#include "widget.d/widget.inc"

void drain_todolist(Widget widget,int|void level)/*{{{*/
{
	//werror("drain_todolist of %s%s %s\n"," "*level,widget->type,widget->id);
	multiset done=(<>);
	array sn_array=({}),cmd_array=({}),object_array=({});;
	void walk(Widget widget){
		foreach(widget->todo,array a){
			sn_array+=({a[0]});
			cmd_array+=({a[1..]});
			object_array+=({widget});
		}
	...
	walk(widget);
	sort(sn_array,cmd_array,object_array);
	for(int i=0;i<sizeof(sn_array);i++){
		if(!done[object_array[i]]){
			array a=cmd_array[i];
			foreach(a,mixed v){
				if(objectp(v))
					done[v]=1;
			}
			string info=handle_todolist_cmd(@a);
		}
	}
	widget->clear_todolist();
#if 0
	foreach(widget->todo,array a){
		foreach(a,mixed v){
			if(objectp(v))
		...
		string info=handle_todolist_cmd(@a);
		//werror("drain_todolist: %s\n",info);
	}
	//werror("widget->items=%O\n",widget->items);
	foreach(widget->init_items,Widget item){
		if(!done[item]){
			drain_todolist(item,level+1);
		}
	}
	widget->clear_todolist();
#endif
...
PUBLIC _widget_move_(string widget,string container,int pos,mixed ... cmdline)/*{{{*/
{
	Widget w=id2widget[widget];
	Widget c=id2widget[container];
	int oldpos=search(c->items,w);
	ASSERT(oldpos>=0);
	w->_father->_delete_internal_(oldpos);
	c->_insert_internal_(pos,w);
	return command(cmdline[0],cmdline[1..]);
}/*}}}*/
	PUBLIC _widget_update_data_internal_(string id,string data)/*{{{*/
	{
		string olddata=id2widget[id]->data;
		id2widget[id]->_set_data_internal_(data);
		id2widget[id]->_set_init_data_internal_(data);
		werror("_widget_update_data_internal_: %s -> %s\n",olddata,data);
		//function f=widget2onchange[id2widget[id]];
		function f=id2widget[id]->on_change;
		werror("_widget_update_data_internal_: f=%O\n",f);
		if(f){
			werror("_widget_update_data_internal_: always_call_on_change=%O\n",id2widget[id]->always_call_on_change);
			if(olddata!=data||id2widget[id]->always_call_on_change){
				f(olddata,data);
			}
		}
	}/*}}}*/

	PUBLIC _widget_onclick_(string widget,mixed ... args)/*{{{*/
	{
		werror("_widget_onclick_ enter\n");
		Widget w=id2widget[widget];
		//function onclick=widget2onclick[w];
		function onclick=w->on_click;
		int res;
		if(onclick){
			werror("onclick=%O\n",onclick);
			werror("_widget_onclick_ call onclick\n");
			res=onclick(@args);
		}
		return res;
	}/*}}}*/

	//private mapping widget2onchange=set_weak_flag(([]),Pike.WEAK);

	void set_widget_onchange(Widget widget,function(string,string:void) cb,int|void always)
	{
		widget->on_change=cb;
		widget->always_call_on_change=always;
		//widget2onchange[widget]=cb;
	}
}
DAEMON:
